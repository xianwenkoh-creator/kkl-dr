...
`earthworks/index.html` — the QS's bulk-excavation method as a tool, for both kinds of drawing set.
From a topographical survey it reads the spot levels and snaps each to its marker, reads the E/N grid
(or the declared scale, or a two-click calibration), lets the QS draw excavation zones and set each
zone's final excavation level (typed, picked off the sheet, from the levels found on the design PDF,
or derived from a slab level less deductions), then computes cut and fill per zone by depth band.
...
batter, giving the cut per platform, on the slopes, by depth band and by construction stage. Colour
overlay for checking, CSV export and a checked-by stamp. `earthworks/engine.js` holds the arithmetic
and is tested with `npm test` against two synthetic sheets with independent truth volumes (see
`earthworks/README.md`).
